<script setup lang="ts">
	...
	function initAnchor() {
		anchor.value = [
			{
				scrollTop: title.value.offsetTop,
				name: '风格',
			},
			{
				scrollTop: uml.value.offsetTop,
				name: '原理',
			},
			{
			...
			{
				scrollTop: codeAnchor.value.offsetTop,
				name: '源码',
			},
			{
				scrollTop: fileInput.value.offsetTop,
				name: 'Try',
			},
		];
	}
	...
	<div class="w-full font-mono h-fit" ref="ctx">
		...
		<main class="flex flex-col justify-around">
			<div class="mb-24">
				...
				<br />
				<article ref="type" class="px-8 text-xl font-bold indent-8 text-zinc-500">
					{{ introduce }}
				</article>
			</div>
			<div class="mb-24">
				...
				<br />
				<img ref="uml" class="liushi-img" lazy :src="imgurl" />
			</div>
			<div class="mb-24">
				<div class="title">源代码结构</div>
				<br />
				<img ref="source" class="liushi-img" :src="uml" alt="" />
			</div>
			<div class="mb-24" v-highlight>
				<div ref="codeAnchor" class="mb-6 title">关键代码</div>
				<pre>
		...
		<ul class="fixed transition-all bottom-44" style="left: 93%">
			<li
				...
				:key="index"
				@click="$emit('onScrollTo', anchorItem.scrollTop)"
			>
				{{ anchorItem.name }}
			</li>
			<li @click="$emit('onScrollTo', 0)">Top</li>
		</ul>
	</div>
</template>
...
<style scoped>
	...
	ul li {
		cursor: pointer;
		@apply block px-4 py-4 rounded-lg font-mono font-bold text-lg bg-teal-400 mb-2 text-slate-100;
	}
</style>
