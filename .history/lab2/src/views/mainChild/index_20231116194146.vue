<script setup lang="ts">
	import { defineAsyncComponent, ref, onActivated, onMounted, getCurrentInstance } from 'vue';
	...
	onMounted(() => {
		console.log(getCurrentInstance()?.refs.Comp);
	});
	onBeforeRouteLeave((to, from, next) => {
		if (Comp.value) {
			console.log(Comp.value.scrollTop);
			scrollHistory.value = Comp.value.scrollTop;
		}
		next();
	});
	onActivated(() => {
		console.log(scrollHistory.value);
		if (scrollHistory.value && Comp.value) {
			Comp.value.scrollTop = scrollHistory.value;
		}
	});
	init();
</script>
